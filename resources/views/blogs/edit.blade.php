...
@section('content')
    ...
    <form action="{{ route('blogs.update', ['blog' => $blog]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="description" id="description" rows="2">{{ $blog->description }}</textarea>
            @error('description')
                <p class="text-danger">
                    <small> {{ $message }} </small>
                </p>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="content" id="content" rows="10">{{ $blog->content }}</textarea>
            @error('content')
                <p class="text-danger">
                    <small> {{ $message }} </small>
                </p>
